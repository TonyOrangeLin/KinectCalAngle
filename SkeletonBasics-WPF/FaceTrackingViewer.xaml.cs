...
namespace Microsoft.Samples.Kinect.SkeletonBasics
{
    ...
    {
        public static Boolean isFaceTracked;
        public static Vector HeadTiltVector;
        public static Vector HeadSpinVector;
        ...
        public void FaceTrackFramesReady(KinectSensor sensor, byte[] colorImage, short[] depthImage, Skeleton[] skeletonData,SkeletonFrame skeletonFrame)
        {          
            try
            {                
                // Update the list of trackers and the trackers with the current frame information
                ...
                foreach (Skeleton skeleton in skeletonData)
                {
                        ...
                        || skeleton.TrackingState == SkeletonTrackingState.PositionOnly)
                    {
                        ...
                        if (this.trackedSkeletons.TryGetValue(skeleton.TrackingId, out skeletonFaceTracker))
                        {
                            skeletonFaceTracker.OnFrameReady(sensor, ColorImageFormat.RgbResolution640x480Fps30, colorImage, DepthImageFormat.Resolution640x480Fps30, depthImage, skeleton);
                            skeletonFaceTracker.LastTrackedFrame = skeletonFrame.FrameNumber;
                        }
                    }
                }
        ...
        private class SkeletonFaceTracker : IDisposable
        {
            ...
            public void DrawFaceModel(DrawingContext drawingContext)
            {
               ...
               HeadTiltVector = new Vector(faceModelPts[10].X - faceModelPts[0].X, faceModelPts[10].Y - faceModelPts[0].Y);
               HeadSpinVector = new Vector(facePoints3D[0].X - facePoints3D[25].X, facePoints3D[0].Z - facePoints3D[25].Z);
